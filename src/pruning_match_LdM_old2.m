function ind_keep = pruning_match_LdM(id_orig, firmid, year_all)
...
while n_of_bad_obs >= 1
    ...
    firmid_y = findgroups(firmid_all, year_all);
    [GC, GR] = groupcounts([id_all, firmid_y]);
    ...
    % Filter the sample with movers and stayers
    sel_all = ismember(firmid_all, firmlst);
    ...
    disp(['Size pruning data, ', num2str(size(id_all, 1))])
