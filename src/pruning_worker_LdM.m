function [ind_keep] = pruning_worker_LdM(id, firmid, year_all)
...
while n_of_bad_workers >= 1 || sum(ind_drop_LdM) > 0 || sum(ind_drop_unique) >0 
    ...
    %articulation point
    [~, artic_points] = biconncomp(G); % Matlab built in function
    ...
    % Remove observations that correspond to articulation points
    sel = ~ismember(id,bad_workers);
