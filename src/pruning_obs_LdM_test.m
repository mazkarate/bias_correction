function [ind_keep] = pruning_obs_LdM_test(id, firmid, year_all)
...
while n_of_bad_obs >= 1 || sum(ind_drop_LdM) > 0 || sum(ind_drop_unique) >0 
    ...
    n_firm_mover_before = length(unique(firmid));
    disp(['Number of firms for movers: ', num2str(n_firm_mover_before)])
    ...
    disp(['Size after leave-one-out: ',num2str(sum(sel_all))])
    ...
    disp(['Size after LdM filter: ',num2str(size(id_obs_all,1))])
    ...
    disp(['Size after one-timer filter: ',num2str(size(id_obs_all,1))])
