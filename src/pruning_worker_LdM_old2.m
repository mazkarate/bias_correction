function ind_keep = pruning_worker_LdM(id, firmid, year)
...
while n_of_bad_workers >= 1
    ...
    firmid_0 = firmid_0(~ind_drop_LdM);
    ...
    firmid_0 = firmid_0(~ind_drop_unique);
    year = year(~ind_drop_unique);



    %%% Articulation points in the connected set
    %find movers
    J=max(firmid);
    gcs = [NaN; id(1:end-1)];
    gcs = id~=gcs;
...
%%% Get final index: belong to connected set AND survive LdM filters
ind_keep = ind_keep_LdM & ind_keep_LdM;
