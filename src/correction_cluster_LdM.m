function [plugin, delta, corrected, decomp_pi, decomp_b, dim_fe, NT] = correction_cluster_LdM(y, id, firmid, cluster, year, other_fe, mat_controls, n_lev,...    
    ...
    firmid = firmid(i_sort_size_C); cluster = cluster(i_sort_size_C); year = year(i_sort_size_C);
   ...
   % Plugin estimates including LdM moment
    ...
    % Run bootstrap
    tic
    [delta, corrected] = bootstrap_cluster_LdM(n_boot, B_plus_par.Value, B_minus_par.Value,...
