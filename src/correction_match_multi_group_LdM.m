function [plugin, delta, corrected, decomp_pi, decomp_b, dim_fe, NT] = correction_match_multi_group_LdM(y, id, firmid, year, group, other_fe, mat_controls, n_lev, n_boot,...
    ...
    clear year dict_id_firmid_weight GC GR i_sort
	if ind_light ==1
		% Clear memory from base and caller workspace
		evalin('base', 'year = [];'); evalin('caller', 'year = [];')
	end

    % Create sparse matrix of firmid_y_sel to compute sum of worker fixed effects per firmid*year by matrix
    % multiplication
    ...
    % Clean memory
    clear pe_tilde fe_tilde b weights av_coworkers sum_pe
